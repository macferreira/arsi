...
You can generate networks using several different models, starting with the ErdÃ¶s-Renyi (the "randomest") network model, and a few variations. 
...
ERDÃ–S-RENYI constructs a random graph by adding an edge between each pair of nodes with probability PROB-LINK.

INTRODUCTION similar to ERDÃ–S-RENYI, however, random edges are added only with probability (1 - PROB-INTRO). The rest of the time the nodes try to get one of their friends to introduce them to someone they know. 
